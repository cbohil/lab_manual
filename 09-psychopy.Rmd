...
The built-in Python->JS translator is good but not perfect. Be sure you are using f-strings (see above).
\
\

(@) I'm trying to repeat trials when the participant doesn't provide a response/correct response. It works in Python but not JS, what's up?

JS's default trial handler changes the name of whatever loop is currently looping to be "trials". So, if you're using the code shown above (In End Routine), you'll need to change the code in the JS window from `repeatTrial.finished = false;` to `trials.finished=false;`
\
\
...
(@) How do I integrate my experiment with SONA for automatic credit granting?

See the guides on [SONA's help page](https://www.sona-systems.com/help/psychopy.aspx) and in the [PsychoPy documentation](https://www.psychopy.org/online/onlineParticipants.html#recruitingonline). 
\
\

### Miscellaneous

(@) How do I cite PsychoPy?

See the [about page](https://www.psychopy.org/about/index.html) of the documentation for the latest, most relevant citation.
